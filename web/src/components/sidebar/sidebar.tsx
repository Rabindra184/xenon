      ...
      <nav className="flex-1 flex flex-col gap-1 w-full items-center">
        {visibleItems.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={`absolute left-0 top-1 bottom-1 w-[3px] rounded-r transition-all ${active ? 'bg-[var(--color-accent)]' : 'bg-transparent'}`}
              />
              <Icon
                className={`h-[18px] w-[18px] transition-colors ${active ? 'text-[var(--color-accent)]' : 'text-[var(--text-dim)] group-hover:text-[var(--text)]'}`}
