...
export class ${pageName} {
  ...
  ${elements.map(el => `/** ${el.type} detected via ${el.strategy} */
  private ${el.name} = () => this.driver.findElement('${el.strategy}', '${el.locator}');`).join('\n\n  ')}
  ...
  async tap${elements[0]?.name || 'Element'}() {
    ...
    ${elements.map(el => `@AndroidFindBy(${el.strategy} = "${el.locator}")
    private MobileElement ${el.name.toLowerCase()};`).join('\n\n    ')}
}`;
        }
...
class ${pageName}:
    ...
    ${elements.map(el => `def ${el.name.toLowerCase()}(self):
        return self.driver.find_element_by_${el.strategy.replace('-', '_')}("${el.locator}")`).join('\n\n    ')}
`;
    };
                    ...
                    elements.map(el => (
                        <div key={el.id} className="pom-element-item animate-fade-in">
                            <div className="pom-item-info">
                                ...
                                <span className="pom-item-meta">{el.strategy}: {el.locator.substring(0, 20)}...</span>
                            </div>
                            <button
                                className="pom-item-delete"
