...
All tools follow appium-mcp conventions (zod schemas, `authToken` credential arg handled by the auth plugin). Scope requirements shown per tool. In hosted mode the plugin additionally enforces **explicit `sessionId` on all session-scoped tools** — core appium-mcp tools included — rejecting omissions with `session_id_required` (§2.6). The three orchestration tools (`xenon_run_flow`, `xenon_orchestration_status`, `xenon_cancel_orchestration`) are **post-GA** (§5) — designed here so the contract is stable, shipped when the orchestrator ships.
...
- **Named spikes (exit criteria, not "possibly"):** (a) runner-Bearer — `@appclaw/runner` MCP client → gateway with `Authorization: Bearer`, fallback documented (§3); (b) capability-gate coverage — all three session-creation paths inject `xenon:options.sessionToken`, including on hub-*node* labs.
...
3. **Healed-step precision** — % of heals a human accepts via "accept healed selector" (§2.7); the false-pass alarm (target ≥90%, alert below 75%).
