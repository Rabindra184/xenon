...
* Run the Appium server with _appium-xenon_ plugin enabled.

```
appium server -ka 800 --use-plugins=xenon --config ./hub-config.json -pa /wd/hub
```

### Node:

Node is a remote machine that consists of devices and appium server running with xenon active. It receives requests from the hub in the form of W3C test commands and executes them using WebDriver
...
* Run the Appium server with _appium-xenon_ plugin enabled in the Node machine.
