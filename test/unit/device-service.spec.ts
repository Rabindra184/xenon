...
describe('Get device', () => {
  before('Set devices in memory', async () => {
    ...
    for await (const device of devices) {
      (await XenonDatabase.DeviceModel).insert({
        ...device,
      });
    }
  });
  ...
  it('Get android device based on filter with minSDK', async () => {
    ...
    expect(semver.gte(sdk10 as semver.SemVer, sdk10 as semver.SemVer)).to.be.true;
    expect(semver.gte(sdk101 as semver.SemVer, sdk10 as semver.SemVer)).to.be.true;
    expect(semver.gte(sdk10 as semver.SemVer, sdk11 as semver.SemVer)).to.be.false;
