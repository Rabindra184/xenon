...
describe('Healing Orchestrator - Cascade Strategy', () => {
  ...
  it('should cascade to OCR (Tier 3) when Fuzzy XML (Tier 2) fails to find a high-confidence match', async () => {
    ...
    const emptyPageSource =
      '<?xml version="1.0" encoding="UTF-8"?><AppiumAUT><XCUIElementTypeWindow /></AppiumAUT>';
    ...
    const ocrProvider = (orchestrator as any).providers.find(
    ...
    console.log(
      `    ✓ Cascaed successful! Fuzzy XML failed, but OCR found the element: ${result?.message || 'OCR result'}`,
    );
  });
