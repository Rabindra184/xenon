...
describe('E2E Forward Request', () => {
  ...
  it('can forward session request to node', async () => {
    ...
    expect(axios.get(`${hub_url}/xenon`)).to.eventually.equal(200, 'hub should be running');
    expect(axios.get(`${node_url}/xenon`)).to.eventually.equal(200, 'node should be running');

    // all devices
    const allDevices = (
      await axios.get(`http://${hub_config.bindHostOrIp}:${HUB_APPIUM_PORT}/xenon/api/device`)
    ).data;
