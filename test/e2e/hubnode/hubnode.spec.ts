...
describe('E2E Hub and Node', () => {
  ...
  const nodeProcess = pluginE2EHarness({
    ...
    pluginSource: 'local',
    pluginSpec: PLUGIN_PATH,
    appiumHome: APPIUM_HOME_NODE as string,
  });
  ...
  it('should have devices on the hub', async () => {
    ...
    const res = await axios.get(`http://${APPIUM_HOST}:${HUB_APPIUM_PORT}/xenon/api/device`);
  ...
  it('serve xenon endpoint when test is still running', async () => {
    await waitForHubAndNode();

    driver = await remote({ ...WDIO_PARAMS, capabilities } as Options.WebdriverIO);

    // check xenon endpoint using axios
    const res = await axios.get(`http://${APPIUM_HOST}:${HUB_APPIUM_PORT}/xenon`);
    expect(res.status).to.equal(200);
  });
  ...
  it('Clean pending session when session failed to start', async () => {
    ...
    const res = await axios.get(`http://${APPIUM_HOST}:${HUB_APPIUM_PORT}/xenon/api/queue/length`);
    expect(res.status).to.equal(200);
