...
export class WDAClient {
    ...
    private wdaConnectionCache: Map<string, { host: string; pathPrefix: string; sessionId?: string }> = new Map();
    private commandQueues: Map<string, Promise<any>> = new Map();
    private typeBuffers: Map<string, { text: string; timer: NodeJS.Timeout | null; pending: boolean }> = new Map();
    ...
    private async executeSerializedCommand<T>(udid: string, action: () => Promise<T>): Promise<T> {
        ...
        const nextInQueue = currentQueue.catch(() => { }).then(() => action()).finally(() => {
    ...
    async sendWDACommand(udid: string, method: 'get' | 'post', endpoint: string, data?: any): Promise<any> {
        if (['/status', '/health'].includes(endpoint) || (method === 'get' && endpoint === '/sessions')) {
            return this.performWDACommand(udid, method, endpoint, data);
        }
        return this.executeSerializedCommand(udid, () => this.performWDACommand(udid, method, endpoint, data));
    }

    private async performWDACommand(udid: string, method: 'get' | 'post', endpoint: string, data?: any): Promise<any> {
        ...
        const port = (streamStatus?.status === 'running' || streamStatus?.status === 'starting') ? streamStatus.wdaPort : (device as any).wdaLocalPort;
        ...
        for (const prefix of prefixes) {
            try {
                ...
                const res = method === 'post' ? await axios.post(url, data || {}, { timeout: 10000 }) : await axios.get(url, { timeout: 10000 });
                const sid = res.data?.sessionId || res.data?.value?.sessionId;
                if (sid && sid !== cached?.sessionId) {
                    this.wdaConnectionCache.set(cacheKey, { host: '127.0.0.1', pathPrefix: `/session/${sid}`, sessionId: sid });
    ...
    async ensureWDASession(udid: string, port: number): Promise<string | null> {
        try {
            const res = await axios.post(`http://127.0.0.1:${port}/session`, { capabilities: { alwaysMatch: { bundleId: 'com.apple.springboard' } } }, { timeout: 10000 });
            const sid = res.data?.sessionId || res.data?.value?.sessionId;
            if (sid) {
                this.wdaConnectionCache.set(`${udid}:${port}`, { host: '127.0.0.1', pathPrefix: `/session/${sid}`, sessionId: sid });
                Container.get(IOSStreamService).setWDASessionId(udid, sid);
                return sid;
            }
    ...
    async tap(udid: string, x: number, y: number): Promise<void> {
        try { await this.sendWDACommand(udid, 'post', '/actions', { actions: [{ type: 'pointer', id: 'finger1', actions: [{ type: 'pointerMove', duration: 0, x, y }, { type: 'pointerDown', button: 0 }, { type: 'pause', duration: 50 }, { type: 'pointerUp', button: 0 }] }] }); }
        catch (e) { await this.sendWDACommand(udid, 'post', '/wda/tap', { x, y }); }
    }

    async swipe(udid: string, x: number, y: number, ex: number, ey: number, duration: number): Promise<void> {
        try { await this.sendWDACommand(udid, 'post', '/wda/swipe', { startX: x, startY: y, endX: ex, endY: ey, delay: duration / 1000 }); }
        catch (e) { await this.sendWDACommand(udid, 'post', '/actions', { actions: [{ type: 'pointer', id: 'finger1', actions: [{ type: 'pointerMove', duration: 0, x, y }, { type: 'pointerDown', button: 0 }, { type: 'pause', duration: 100 }, { type: 'pointerMove', duration: 500, x: ex, y: ey }, { type: 'pointerUp', button: 0 }] }] }); }
    }
    ...
    private async flushTypeBuffer(udid: string) {
        ...
        if (!b || !b.text) return;
        const t = b.text; b.text = ''; b.timer = null; b.pending = true;
        try { await this.sendWDACommand(udid, 'post', '/wda/type', { text: t }); }
        finally { b.pending = false; if (b.text) setTimeout(() => this.flushTypeBuffer(udid), 50); }
    }

    async pressKey(udid: string, key: string | number): Promise<void> {
        const n = key.toString().toLowerCase();
        if (['home', '3'].includes(n)) { try { await this.sendWDACommand(udid, 'post', '/wda/homescreen', {}); } catch (e) { await this.sendWDACommand(udid, 'post', '/wda/pressButton', { name: 'home' }); } return; }
        await this.sendWDACommand(udid, 'post', '/wda/pressButton', { name: n === 'backspace' ? 'delete' : n });
    }
    ...
    async getClipboard(udid: string): Promise<string> {
        try {
            await this.sendWDACommand(udid, 'post', '/wda/apps/activate', { bundleId: 'com.apple.springboard' });
