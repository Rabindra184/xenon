...
export class UniversalMjpegProxy {
    ...
    public proxyRequest(req: Request, res: Response) {
        ...
        log.debug(`[MjpegProxy] New client joined stream: ${this.mjpegUrl}. Total clients: ${this.clients.size}`);
    ...
    private startSource() {
        ...
        this.sourceRequest.on('error', (err: any) => {
            ...
            if (err.code === 'HPE_INVALID_CONSTANT' || err.message.includes('Parse Error')) {
                log.info(`[MjpegProxy] Detected raw (head-less) MJPEG stream from source. Switching to Raw Mode.`);
                this.startRawSource();
            } else {
    ...
    public stop() {
        ...
        log.info(`[MjpegProxy] Terminating ${this.clients.size} client connections for ${this.mjpegUrl}`);
        for (const client of this.clients) {
            try {
                client.end();
            } catch (e) { /* ignore */ }
