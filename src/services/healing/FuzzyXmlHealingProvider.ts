...
export class FuzzyXmlHealingProvider implements HealingProvider {
    ...
    async heal(context: HealingContext): Promise<HealedElement | null> {
        ...
        try {
            ...
            for (const node of nodes) {
                ...
                if (score > highestScore && score > 0.5) { // Threshold 50% for POC robustness
                    highestScore = score;
                    bestMatch = node;
                }
            }
            ...
            if (bestMatch) {
                ...
                this.logger.info(`✅ Found fuzzy match with score ${highestScore.toFixed(2)}: ${recommendedXpath}`);
    ...
    private extractKeywords(selector: string): string[] {
        ...
        return cleaned.split(/\s+/)
            .filter(s => s.length > 2 && !blacklist.includes(s.toLowerCase()));
    }

    private calculateScore(node: Element, keywords: string[], etalon: LocatorSignature | null = null): number {
        ...
        const textTarget = etalon ? (etalon.attributes['text'] ? [etalon.attributes['text']] : keywords) : keywords;
