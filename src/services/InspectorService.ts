...
export class InspectorService {
    ...
    async getSnapshot(udid: string): Promise<InspectorSnapshot> {
        try {
            ...
            const [screenshotBase64, xmlSource] = await Promise.all([
                manager.getScreenshot ? manager.getScreenshot(udid) : Promise.reject('Screenshot not supported'),
                manager.getPageSource ? manager.getPageSource(udid) : Promise.reject('Page source not supported'),
            ]);
            ...
            if (!device.screenWidth || !device.screenHeight) {
                try {
                    if (manager.getAdditionalDeviceInfo) {
                        ...
                        Container.get<any>('DeviceStore').updateDevice(udid, device.host, additional).catch(() => { });
                    }
                } catch (e) {
    ...
    private parseXmlHierarchy(xml: string, platform: string): InspectorNode {
        try {
            ...
            return { name: 'root', type: 'error', rect: { x: 0, y: 0, width: 0, height: 0 }, xpath: '/', children: [], attributes: {}, suggestedLocators: [], suggestedActions: [] };
        }
    }
    ...
    private transformAndroidNode(node: any, root: any): InspectorNode {
        ...
        const children = Array.isArray(node.node)
            ...
            : node.node ? [this.transformAndroidNode(node.node, root)] : [];

        const attributes: Record<string, any> = {};
        Object.entries(node).forEach(([key, value]) => {
            // Filter out internal structural keys and non-primitive values
            if (key !== 'node' && key !== 'xpath' && (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')) {
    ...
    private transformIosNode(node: any, root: any): InspectorNode {
        ...
        Object.entries(data || {}).forEach(([key, value]) => {
            ...
            if (!key.startsWith('XCUIElement') && (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')) {
                attributes[key] = value;
            }
        });
    ...
    private generateActions(node: InspectorNode): SuggestedAction[] {
        ...
        if (type.includes('button') || type.includes('imageview') || type.includes('icon') || node.attributes.clickable === 'true' || node.attributes.enabled === 'true') {
            actions.push({
                action: 'click',
        ...
        if (type.includes('edit') || type.includes('textfield') || type.includes('input') || type.includes('search')) {
            actions.push({
                action: 'sendKeys',
    ...
    private generateLocators(node: InspectorNode, platform: string, root: any): LocatorSuggestion[] {
        ...
        const accId = node.attributes.accessibilityId || node.attributes.name || node.attributes.resourceId;
        if (accId) {
